# Installing WeWrite for OpenClaw
...
```bash
git clone --depth 1 https://github.com/oaker-io/wewrite.git ~/.openclaw/wewrite
```
...
ln -s ~/.openclaw/wewrite ~/.openclaw/skills/wewrite
```

### 3. Install Python Dependencies

```bash
cd ~/.openclaw/wewrite && pip install -r requirements.txt
```
...
If successful, OpenClaw will automatically recognize and invoke the WeWrite skill.
...
- Image API key (for image generation)
